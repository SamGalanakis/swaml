...
// Stub for iOS - show_error just logs
#[cfg(any(target_arch = "wasm32", target_os = "ios"))]
fn show_error(msg: &str) -> Result<()> {
    baml_log::error!("{}", msg);
    Ok(())
}
...
impl InitArgs {
    ...
    // iOS/WASM version without UI dependencies
    #[cfg(any(target_arch = "wasm32", target_os = "ios"))]
    pub fn run(&self, defaults: super::RuntimeCliDefaults) -> Result<()> {
        // Detect project type if not explicitly provided
        let output_type = if let Some(client_type) = self.client_type {
        ...
        baml_log::info!("✨ Created new BAML project in {}", baml_src.display());
        baml_log::info!("📚 Follow instructions at https://docs.boundaryml.com/ref/overview to get started!");
