...
impl AggregateRenderer {
    ...
    fn new(output_format: &crate::cli::testing::OutputFormat, _junit_path: Option<&String>) -> Self {
        // On iOS/WASM, always use github output format (no indicatif/junit support)
        let renderers: Vec<Box<dyn RenderTestExecutionStatus>> = vec![Box::new(
            output_github::GithubTestExecutionStatusRenderer::new(),
        )];
        Self { renderers }
    }
